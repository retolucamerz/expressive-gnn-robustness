# code from https://github.com/KarolisMart/AgentNet/blob/main/smp_models/ppgn.py
# initially from https://github.com/cvignac/SMP
...
class PowerfulLayer(nn.Module):
    def __init__(
        self, in_feat: int, out_feat: int, num_layers: int, activation=nn.LeakyReLU()
    ):
        super().__init__()
        ...
        self.m1 = nn.Sequential(
            *[
                nn.Linear(in_feat if i == 0 else out_feat, out_feat)
                if i % 2 == 0
                else activation
    ...
    def forward(self, x, mask):
        ...
        out = (
            out / norm
        )  # Fix from SPECTRE - Normalize to make std~1 (to preserve gradient norm, similar to self attention normalization)
...
class Powerful(nn.Module):
    def __init__(
        ...
        initial_conv = PowerfulLayer(
            input_features, hidden, layers_per_conv, activation
        )
            ...
            if isinstance(m, nn.Linear):
                if self.activation_function == "gelu":
                    ...
                    nn.init.kaiming_uniform_(
                        m.weight, a=self.negative_slope, nonlinearity="leaky_relu"
                    )
    ...
    def forward(self, data, x=None, adj=None, edge_attr=None, mask=None):
        x, mask, adj, edge_attr = to_dense_data(data, x=x, adj=adj, mask=mask, edge_attr=edge_attr)
        ...
        u = torch.cat(
            [
                adj[..., None],
                edge_attr,
                torch.diag_embed(x.transpose(-2, -1), dim1=1, dim2=2),
            ],
            dim=-1,
        )
        u = u * mask
...
@with_metrics
@with_encoders
class PowerfulNoEdgeFeat(Powerful):
    def forward(self, data, x=None, edge_attr=None, adj=None, mask=None):
        # Convert to dense representation
        if x is None or mask is None:
            x, mask = to_dense_batch(data.x, data.batch)  # batch, N, num_node_feats
        if adj is None:
