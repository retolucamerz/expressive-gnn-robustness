...
from models.GINE import GINEConv as PyGINEConv
...
class GINEConv(torch.nn.Module):
    def __init__(self, in_dim, emb_dim):
        ...
        self.layer = PyGINEConv(nn=mlp, train_eps=False)
    ...
    def __init__(
        ...
        if num_random_features > 0:
            ...
            self.initial_layers = torch.nn.ModuleList(
                [
                    GraphConv(in_dim, emb_dim // 2),
                    GraphConv(emb_dim // 2, emb_dim - num_random_features),
                ]
            )
